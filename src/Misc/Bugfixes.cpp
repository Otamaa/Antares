...
{
	GET(AbstractClass*, _This, ECX);
	GET(AbstractClass*, _That, EBX);

	auto nThisCoords = _This->GetCoords();
	auto nThisC = Point2D{ nThisCoords.X,nThisCoords.Y };
	auto nThatCoords = _That->GetCoords();
	auto nThatC = Point2D{ nThatCoords.X,nThatCoords.Y };
	auto nDistance = (int)nThisC.DistanceFrom(nThatC);

	if (nDistance >= std::numeric_limits<int>().max())
		R->EAX(std::numeric_limits<int>().max());
	else
		R->EAX(nDistance);
...
DEFINE_HOOK(5F6560, AbstractClass_Distance2DSquared_2, 5)
{
	GET(AbstractClass*, _This, ECX);
	GET_STACK(CoordStruct*, _That, 0x4);

	auto nThisC = Point2D{ _This->GetCoords().X,_This->GetCoords().Y };
	auto nThatC = Point2D{ _That->X,_That->Y };
	auto nDistance = (int)nThisC.DistanceFrom(nThatC);
...
//
DEFINE_HOOK(4521C8, BuildingClass_Disable_Temporal_Factories, 6)
{
...
DEFINE_HOOK(452218, BuildingClass_Enable_Temporal_Factories, 6)
{
	GET(BuildingClass*, pThis, ECX);
...
// https://ares-developers.github.io/Ares-docs/bugfixes/type2/freeunit.html?highlight=missions
// Backport : Otamaa
// Added more conditions , especially for AI better to set is as Hunt
DEFINE_HOOK(446E9F, BuildingClass_Place_FreeUnit_Mission, 6)
{
	GET(UnitClass*, pFreeUnit, EDI);

	Mission nMissions;
	if (pFreeUnit->Type->Harvester || pFreeUnit->Type->ResourceGatherer)
	{
		nMissions = Mission::Harvest;
	}
	else
	{
		nMissions = (!pFreeUnit->Owner->ControlledByHuman())
